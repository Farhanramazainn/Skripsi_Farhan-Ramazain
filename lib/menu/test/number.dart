...
class _NumberPageState extends State<NumberPage> {
  ...
  Future<void> _initializeApp() async {
    try {
      setState(() => _loadingMessage = 'Memuat daftar angka...');
      await _loadLabels();
  ...
  Future<void> _loadLabels() async {
    try {
      final String labelsData = await rootBundle.loadString('assets/model/labels_angka_v3.txt');
      _labels = labelsData.split('\n').map((e) => e.trim()).where((e) => e.isNotEmpty).toList();
      setState(() => _isLabelsLoaded = true);
    } catch (_) {
      // Fallback: Generate 0-9 labels (no labels.txt needed)
      _labels = List.generate(10, (i) => i.toString());
      setState(() => _isLabelsLoaded = true);
      debugPrint('Using fallback 0-9 labels');
    }
  }

  Future<void> _loadModel() async {
    try {
      interpreter = await Interpreter.fromAsset('assets/model/model_sibi_angka_v3.tflite');
    } catch (e) {
  ...
  void _generateRandomQuestions() {
    // Create randomized questions from labels (keep all 10 numbers 0-9)
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text('Tebak Angka SIBI', style: GoogleFonts.poppins()),
          backgroundColor: Colors.blue,
          foregroundColor: Colors.white,
          leading: IconButton(
